-- Smart Align Post - STATIC MULTI UI
-- Primera selección = MASTER. Siguientes = SOURCES.
-- Analiza offline y aplica corrigiendo D_STARTOFFS, sin mover D_POSITION.
...
local masterTake = nil
local masterPath = nil
local masterOffs = 0
local masterRate = 1
...
local function parse_process_output(processResult)
  ...
  return returnCode, output, normalized
end
...
local function analyze_selection()
  ...
  if masterRate <= 0 then
    fail("PLAYRATE inválido en MASTER.")
    return
  end
  ...
  for i = 1, selectedCount - 1 do
    ...
    if not sourcePath then
      fail("SOURCE #" .. i .. ": " .. tostring(sourceErr))
      return
    end
    ...
    if sourceRate <= 0 then
      fail("PLAYRATE inválido en SOURCE #" .. i .. ".")
      return
    end
    ...
    if returnCode ~= 0 then
      fail("El analizador falló en SOURCE #" .. i .. ". " .. tostring(normalized))
      return
    end
    ...
    local targetOffs = masterOffs + (delayMs / 1000.0) * sourceRate / masterRate
...
local function apply_results()
  ...
  if failures > 0 then
    set_status(string.format("APPLY terminó con %d SOURCE(s) con error de verificación.", failures), "error")
    return
  end
...
local function handle_mouse()
  ...
  if down and not lastMouseDown then
    local footerY = gfx.h - 72
    if inside(18, footerY + 8, 150, BUTTON_H, gfx.mouse_x, gfx.mouse_y) then
      analyze_selection()
    elseif inside(180, footerY + 8, 150, BUTTON_H, gfx.mouse_x, gfx.mouse_y) then
      apply_results()
    elseif inside(gfx.w - 168, footerY + 8, 150, BUTTON_H, gfx.mouse_x, gfx.mouse_y) then
      gfx.quit()
      return true
    end
  end
...
-- Abrimos la ventana y hacemos el primer análisis automáticamente.
gfx.init("Smart Align Post — STATIC MULTI", WIN_W, WIN_H)
